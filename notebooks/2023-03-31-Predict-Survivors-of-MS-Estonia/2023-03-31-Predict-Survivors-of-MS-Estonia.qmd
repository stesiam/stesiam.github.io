...
&nbsp; 3.4 [Correlogram](#correlogram) <br>
...
Sex -> factor (men /women)
Category (crew/passinger)
Survived (yes/no)
...
```{r}
table(estonia_passenger_list$Country) %>% sort(decreasing = T)
```
...
```{r}
table(estonia_passenger_list$Age) %>% sort(decreasing = T)
```
