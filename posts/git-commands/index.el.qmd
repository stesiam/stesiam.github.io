...
Έχοντας περιγράψει τι είναι το VCS, θα ήταν χρήσιμο να μελετήσουμε ποιο ήταν το πιο γνωστό ή πιο χρησιμοποιούμενο διαχρονικά. Για να απαντήσουμε, θα ανατρέξουμε στα ιστορικά δεδομένα αναζητήσεων στη μηχανή αναζήτησης της Google και πιο συγκεκριμένα από τη πλατφόρμα  [Google Trends](https://trends.google.com/trends/) που διατηρεί δεδομένα τάσεων αναζητήσεων δημοφιλών όρων.
...
Αν καμία από αυτές τις επιλογές δεν μας ικανοποιεί (π.χ. ανησυχία σχετικά με τους όρους χρήσης τους), υπάρχει και η επιλογή να φιλοξενήσουμε τον κώδικα εμείς οι ίδιοι. Εργαλεία όπως το [Gitea](https://gitea.io/en-us/) μας δίνουν τη δυνατότητα να στήσουμε τη δική μας «πλατφόρμα» σε έναν δικό μας server. Με απλά λόγια ένα δικό μας GitHub, χωρίς εξάρτηση από τις προαναφερόμενες πλατφόρμες. Αυτή η επιλογή αν και ακούγεται καλή, στην πράξη είναι αρκετά σύνθετη και αποκόπτεσαι από τη μεγάλη πλειοψηφία των devs.
...
::: {.column-margin}
![Φωτογραφία του Linus Torvalds το 2018](images/linus.webp)

**Πηγή φωτογραφίας: **Wikimedia Commons, 2021 - Άδεια χρήσης: [CC BY 3.0](https://creativecommons.org/licenses/by/3.0/). Για τη πρωτότυπη φωτογραφία μπορείτε να πατήσετε τον [σύνδεσμο](https://commons.wikimedia.org/wiki/File:Gilbert_Walker.jpg)
:::
...
Το Visual Studio Code είναι σήμερα ο πιο δημοφιλής editor σύμφωνα με την [έρευνα προγραμματιστών του Stack Overflow](https://insights.stackoverflow.com/survey/2021#section-most-popular-technologies-integrated-development-environment), οπότε αποτελεί μία ασφαλή επιλογή αν δεν έχεις ιδιαίτερη προτίμηση.

### Προεπιλεγμένο όνομα κλαδου (branch)

Τον Οκτώβριο του 2020, το GitHub [ανακοίνωσε](https://github.blog/changelog/2020-10-01-the-default-branch-for-newly-created-repositories-is-now-main/) ότι αλλάζει το προεπιλεγμένο όνομα του αρχικού κλάδου (branch) από `master` σε `main`.
