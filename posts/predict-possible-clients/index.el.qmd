...
stack_val_preds <- predict(bank_stack_fit,
                           bank_val,
                           type = "prob") %>%
  bind_cols(bank_val %>% select(y))

upper_bound_stack <- max(stack_val_preds$.pred_ναι, 
                         na.rm = TRUE) * 0.95

stack_threshold <- stack_val_preds %>%
  threshold_perf(
    truth      = y,
    estimate   = .pred_ναι,
...
threshold_lookup <- setNames(
    all_thresholds$threshold,        # ← το σωστό όνομα
