...
ctrl <- control_stack_grid()  # αντί για control_grid(), για stacking
```
...
rf_results <- tune_grid(
  rf_wf, resamples = cv_folds, grid = 10,
  metrics = metric_set(roc_auc, accuracy, f_meas),
  control = ctrl
)

xgb_results <- tune_grid(
  xgb_wf, resamples = cv_folds, grid = 10,
  metrics = metric_set(roc_auc, accuracy, f_meas),
  control = ctrl
)

lgbm_results <- tune_grid(
  lgbm_wf, resamples = cv_folds, grid = 10,
  metrics = metric_set(roc_auc, accuracy, f_meas),
  control = ctrl
)
