...
Όταν θέλουμε να καταθέσουμε χρήματα στην τράπεζα συνήθως οι περισσότεροι έχουν
λογαριασμούς ταμιευτηρίου ή αποταμίευσης. Ένα διαφορετικό είδος
λογαριασμού είναι ο προθεσμιακός, όπου σε αυτή την περίπτωση δεσμεύεται ο πελάτης
...
-   Η Eurobank, προσφέρει μηδενικό επιτόκιο σε λογαριασμούς ταμιευτηρίου, 0.01%
    μέχρι και 0.35% σε λογαριασμούς αποταμίευσης και στις προθεσμιακές ποικίλει
...
αυτά τα προϊόντα. Με βάση τη λογική, το κατάλληλο προφίλ αφορά άτομα με σημαντικό ύψος καταθέσεων και να μην έχουν
...
και η προσαρμογή Boosting μοντέλων είναι αρκετά χρονοβόρα σε σύγκριση με την
...
λάβεις ένα συγκεκριμένο αποτέλεσμα πιθανότατα να είναι εντάξει. Βέβαια, τις
περισσότερες φορές απαιτείται προσεκτικότερος σχεδιασμός της μελέτης. Ένα
...
Αντίστοιχα, θα ορίσω και τη συνάρτηση `univariateQuantitativePlot` για την
...
Είναι προφανές ότι αν έχει μη εξυπηρετούμενες οφειλές το τελευταίο πράγμα που θα σκεφτεί είναι να κάνει αποταμίευση, αλλά να ξεπληρώσει τα χρέη του. Στο δείγμα μόλις 76 άτομα που αντιστοιχούν στο 1.6\% των πελατών εμπίπτει σε αυτή την κατηγορία, γεγονός που καθιστά να έχει νόημα η πρόταση για τη συντριπτική πλειοψηφία.
...
Εδώ στην ουσία έχω το αποτέλεσμα το οποίο προσπαθώ να προβλέψω. Αυτά τα δεδομένα είναι γνωστά από την τωρινή καμπάνια. Με βάση όμως αυτά τις προηγούμενες μεταβλητές - χαρακτηριστικά θα κατασκευάσω ένα μοντέλο πρόβλεψης και αυτά τα μοντέλα θα αξιολογηθούν με βάση αυτά τα δοσμένα αποτελέσματα. 
...
Μία σύνοψη των παραπάνω μπορεί να γίνει και με το παρακάτω διάγραμμα αλληλουχιών. Με τη πρώτη στήλη να δηλώνει την κατανομή στις οικογενειακές καταστάσεις συνδυαζόμενες από το εκπαιδευτικό υπόβαθρο των συμμετεχόντων και καταλήγω στην τρίτη στήλη που είναι και η τελική απάντηση του πελάτη (αν ενδιαφέρεται να ανοίξει προθεσμιακό λογαριασμό).
...
/ 10%). Σε αυτή την περίπτωση μπορώ να χρησιμοποιήσω την εντολή `step_smote()` από
...
προτείνεται να κατασκευάσουμε υποσύνολα των δεδομένων μου με σκοπό να βρω τις
...
Αφού ορίσαμε τα μοντέλα και τις αντίστοιχες ροές εργασίας (workflows), προχωρώ στη βελτιστοποίηση των υπερπαραμέτρων τους. Για τα απλούστερα μοντέλα (Λογιστική Παλινδρόμηση, KNN και Random Forest) χρησιμοποιώ αναζήτηση πλέγματος (`tune_grid()`), όπου δοκιμάζονται συστηματικά διάφοροι συνδυασμοί υπερπαραμέτρων. Για τα μοντέλα gradient boosting (XGBoost και LightGBM), τα οποία διαθέτουν περισσότερες υπερπαραμέτρους και κάθε εκτέλεσή τους είναι υπολογιστικά ακριβότερη, επιλέγω Νπεϋζιανή βελτιστοποίηση (`tune_bayes()`). Σε αυτήν την προσέγγιση, ο αλγόριθμος «μαθαίνει» από τις προηγούμενες δοκιμές και επιλέγει πιο στοχευμένα τον επόμενο συνδυασμό παραμέτρων, εξοικονομώντας χρόνο σε σχέση με την εξαντλητική αναζήτηση.
...
Πέρα από τη μεμονωμένη αξιολόγηση κάθε μοντέλου, εφαρμόζω και μια τεχνική που ονομάζεται stacking. Η ιδέα πίσω από το stacking είναι ότι αντί να επιλέξω ένα μόνο μοντέλο ως τελικό, μπορώ να συνδυάσω τις προβλέψεις πολλών μοντέλων σε ένα μετα-μοντέλο που μαθαίνει ποιο αποδίδει καλύτερα και σε ποιες περιπτώσεις. Στην πράξη, τα μοντέλα που εκπαίδευσα προηγουμένως (Λογιστική Παλινδρόμηση, KNN, Random Forest, XGBoost, LightGBM) τροφοδοτούν τις προβλέψεις τους σε ένα δεύτερο επίπεδο, όπου ένα γραμμικό μοντέλο με L1 κανονικοποίηση αναλαμβάνει να καθορίσει το βάρος που θα δοθεί στο κάθε ένα ή ακόμα και να το αποκλείσει εντελώς αν δεν συνεισφέρει. Με αυτόν τον τρόπο αξιοποιώ τα πλεονεκτήματα κάθε αλγορίθμου χωρίς να δεσμεύομαι σε μία μόνο προσέγγιση. Για την υλοποίηση αυτής της τεχνικής χρησιμοποιώ το πακέτο `{stacks}`, το οποίο ενσωματώνεται ομαλά στο οικοσύστημα `{tidymodels}`.
...
Τέλος, η υπεροχή του μοντέλου LightGBM έχει αποδειχτεί όμως κρίνω απαραίτητη την προσθήκη ενός ακόμη εργαλείου για να κάνει ξεκάθαρη όχι απλώς τη διαφορά στην προβλεπτική τους απόδοση, αλλά κυριώς στα ποιοτικά χαρακτηριστικά αυτής. Βλέπετε, το κάθε μοντέλο έχει τις δικές του δυνάμεις και συνάμα αδυναμίες. Άλλο μοντέλο μπορεί να είναι πιο ευαίσθητο σε κάποιες τιμές και άλλο να κατηγοριοποιεί πιο δύσκολα. 
...
all_preds %>%
    ...
    gt_custom(head_max = 10, use_labels = FALSE) %>%
    tab_header(title = "Πίνακες Σύγχυσης - Confusion Matrices") %>%
...
Ο παραπάνω πίνακας είναι άκρως αποκαλυπτικός για αυτό που θέλω να περιγράψω. Δεν αρκεί η ακρίβεια ως μέτρο σύγκρισης. Σκεφτείτε τι θέλαμε, να βρούμε τους πελάτες που ενδιαφέρονται. Στο κομμάτι των δεδομένων που απομονώσαμε για να ελέγξουμε την ισχύ των μοντέλων υπάρχουν 1000 αρνήσεις και 131 θετικές εκβάσεις. Εγώ θεωρητικά ψάχνω το μοντέλο με τη μεγαλύτερη ακρίβεια, δηλαδή το μεγαλύτερο ποσοστό στα διαγώνια κελιά, όπου για το LightGBM είναι 87.8\%, για το XGBoost 88.4\% και για τη Λογιστική Παλινδρόμηση 61.8\%. Μία βιαστική επιλογή θα ήταν το XGBoost. Ωστόσο, παρατηρώ το εξής ενδιαφέρον ότι αυτό το μοντέλο προβλέπει ότι κανένας δεν ενδιαφέρεται, άρα χάνονται και οι 131 δυνητικοί πελάτες. Επομένως η επιλογή του μοντέλου δεν θα πρέπει να βασιστεί στην καλύτερη ταξινόμηση αλλά στην αποδοτικότερη εύρεση των ενδιαφερόμενων  (ταύτιση των Ναι, πρόβλεψης και πραγματικότητας). Αν δούμε προσεχτικότερα το ίδιο πρόβλημα έχει και το LightGBM μοντέλο με μόλις 22 θετικές προβλέψεις από τις 131 και το μοντέλο λογιστικής παλινδρόμησης να δίνει μία ανέλπιστα καλή απόδοση βρίσκοντας τα 2/3 των ενδιαφερόμενων. Κάπου εδώ βέβαια έχει σημασία να παρατηρήσουμε και πόσο θα πρέπει να απασχοληθούμε για να αντλήσουμε αυτούς τους πελάτες. Το μοντέλο LightGBM προβλέπει 49 δυνητικούς πελάτες - άρα οι οχλήσεις είναι αρκετά μειωμένες σε σχέση με αυτές τις λογιστικής παλινδρόμησης που προσεγγίζουν τις 500.
