...
```{r}
# f1 = parliament %>%
#   dplyr::count(FullName, Party) %>%
#   arrange(-n) %>%
#   dplyr::filter(n>=9) %>%
#   mutate(lastName = extract_name_parts_vec(FullName))
# 
# party_colors = c(
# kke_color = "#FF6666",
# nd_color = "#0492c2",
...
# highchart() %>%
#   hc_chart(type = "bar") %>%
#   hc_title(text = "Top 20 Politicians by Times Elected") %>%
#   hc_xAxis(
#     categories = f1$lastName$last_name,
#     title = list(text = "Politician")
#   ) %>%
...
#     showInLegend = FALSE
#   ) %>%
#   hc_tooltip(pointFormat = "<b>{point.category}</b><br>Times elected: {point.y}")
```
...
highchart() %>%
    ...
    hc_title(text = glue("Most elected MPs of {party}")) %>%
    hc_subtitle(text = glue()) %>%
    hc_xAxis(
      categories = party_df$FullName,
      title = list(text = "Members of Parliament"),
      labels = list(
        style = list(fontSize = "11px", whiteSpace = "normal"),
        useHTML = TRUE
      )
    ) %>%
    hc_yAxis(title = list(text = "Times elected")) %>%
    ...
    hc_tooltip(pointFormat = "<b>{point.category}</b>: {point.y} times elected") %>%
