...
`{gt}` με ορισμένες αλλαγές στην εμφάνιση για να φαίνονται ωραία. Για αυτό
σε πολλούς πίνακες θα δείτε χρήση της `gt_custom()`, αντί της προεπιλεγμένης `gt()`.
...
Θα ήταν εξαιρετικά ενδιαφέρον, δεδομένου ότι υπάρχουν πληροφορίες για τους βουλευτές και για τις κοινοβουλευτικές ομάδες στις οποίες ανήκουν, να κατασκευάσω διαγράμματα σύνθεσης της Βουλής. Μιας και κάνω ιστορική αναδρομή είναι πολύ πιθανό να χρειαστώ παραπάνω από τρία σε αυτό το άρθρο, γεγονός που δικαιολογεί τη δημιουργία μίας συνάρτησης. Η συνάρτηση θα καλείται `plot_parliament_term()` και θα δέχεται ως όρισμα έναν αριθμό που θα δηλώνει ποιας περιόδου θα θέλαμε να διαγραμματίσουμε. Τα δεδομένα μου καλύπτουν από την Α' (1η) περίοδο (1974 - 1977) μέχρι και την ΙΘ' (19η) περίοδο (2023A), συνεπώς το όρισμα της συνάρτησης δέχεται τιμές από το 1 μέχρι το 19. 
...
plot_parliament <- function(data, period_number, lang = "el") {
  ...
  seats_party <- df_period |>
    group_by(abbrNamesEl, fullNamesEl, Color) |>   # ← χωρίς lr_position
    summarise(n = n(), .groups = "drop") |>
    mutate(
      pct = round(100 * n / total_seats, 1),
      lr_position = case_when(               # ← εδώ, μετά το group_by
        abbrNamesEl %in% c("ΚΚΕ", "ΚΚΕ εσ.", "ΕΔΑ")          ~ 1,
        abbrNamesEl %in% c("ΣΥΡΙΖΑ", "ΛΑΕ", "ΔΗΚΚΙ")          ~ 2,
        abbrNamesEl %in% c("ΔΗΜΑΡ", "ΣΠΑΔ")                    ~ 3,
        abbrNamesEl %in% c("ΠΑΣΟΚ", "ΠΟΤΑΜΙ")                  ~ 4,
        abbrNamesEl %in% c("Ανεξάρτητοι", "ΑΔΠ", "ΜέΡΑ25",
                            "ΕΔΗΚ", "ΚΙΔΗΣΟ")                   ~ 5,
        abbrNamesEl %in% c("ΝΔ", "ΔΙΑΝΑ", "ΕΠ", "ΚΝΦ")         ~ 6,
        abbrNamesEl %in% c("ΑΝΕΛ", "ΛΑΟΣ")                     ~ 7,
        abbrNamesEl == "ΧΑ"                                     ~ 8,
        TRUE                                                     ~ 5
      )
    ) |>
  ...
  title_text    <- if (lang == "el") paste0("Σύνθεση Βουλής – Περίοδος ", period_number)
                   else paste0("Parliament Composition – Period ", period_number)
  subtitle_text <- if (lang == "el") paste0("Σύνολο εδρών: ", total_seats,
                                            " · Πλειοψηφία: ", majority)
                   else paste0("Total seats: ", total_seats, " · Majority: ", majority)
  caption_text  <- if (lang == "el") "Πηγή: Βουλή των Ελλήνων"
                   else "Source: Hellenic Parliament"
  ...
  highchart() |>
    ...
    hc_tooltip(
      ...
      pointFormat  = paste0(
        ...
        "<b>{point.name}</b><br/>",
        if (lang == "el") "Έδρες: <b>{point.y}</b> ({point.pct}%)"
        else              "Seats: <b>{point.y}</b> ({point.pct}%)"
      )
    ) |>
    
    hc_add_series(
  name = if (lang == "el") "Έδρες" else "Seats",
  data = purrr::pmap(
    list(seats_party$fullNamesEl,
         seats_party$n,
         seats_party$Color,
         seats_party$abbrNamesEl,
         seats_party$pct),
    ~ list(name  = ..1,
           y     = ..2,
           color = ..3,
  ...
  dataLabels = list(
    enabled         = TRUE,
    format          = "{point.label}",
    backgroundColor = "#000000",      # ← μαύρο φόντο
    borderRadius    = 3,
    padding         = 4,
    style           = list(
      ...
      fontSize    = "11px",
      fontWeight  = "bold",
      textOutline = "none"            # ← αφαίρεση default outline
    )
  ),
  center     = list("50%", "88%"),
  size       = "170%",
...
hc_responsive(rules = list(
  list(
    condition    = list(maxWidth = 768),    # ← κάτω από 768px: κρύψε labels
    chartOptions = list(
      series = list(list(
...
Τα συλλεγμένα δεδομένα αποτελούνται από `r ncol(parliamentGR)` μεταβλητές (στήλες), οι οποίες είναι όλες ποιοτικές. Από αυτές τις πέντε, οι τέσσερις είναι κατηγορικές και μία διατάξιμη μεταβλητή.

::: {.table-responsive}
|    Μεταβλητή    |           Τύπος μεταβλητής           |                 Περιγραφή                  |
|:----------------:|:----------------------:|:------------------------------------:|
|  `Full Name`   | *Ποιοτική* <br> (κατηγορική) | Επώνυμο / Όνομα / Όνομα πατέρα |
|    `Party`     | *Ποιοτική* <br> (κατηγορική) |    Κοινοβουλευτική ομάδα - Κόμμα που εκπροσωπεί     |
| `Constituency` | *Ποιοτική* <br> (κατηγορική) |       Εκλογική περιφέρεια          |
|     `Term`     | *Ποιοτική* <br> (διατάξιμη) |                 Περίοδος θητείας                 |
|     `gender`     | *Ποιοτική* <br> (κατηγορική) |                Εκτίμηση κοινωνικού φύλου με βάση το όνομα    |
...
Έχοντας τακτοποιήσει τις πληροφορίες, θα ξεκινήσω την ανάλυσή μου με μία ιστορική αναδρομή στον αριθμό των κομμάτων που έχουν εκπροσωπηθεί στο Ελληνικό Κοινοβούλιο και έχουν ολοκληρώσει τη θητεία τους ως κοινοβουλευτική ομάδα. Από την πτώση της χούντας, τα κόμματα που συμμετείχαν στο κοινοβούλιο ήταν αρκετά λίγα, και είναι τα πλέον ιστορικά, το ΚΚΕ, η Νέα Δημοκρατία και το ΠΑΣΟΚ. Έτσι η Βουλή του 1981, αποτελούνταν μόλις από τρία κόμματα. 


```{r}
#| label: fig-parl-term-3
#| fig-cap: "Σύνθεση Ελληνικού Κοινοβουλίου Γ´ περιόδου (1981 - 1985)"
#| fig-alt: "Ημικυκλικό γράφημα κοινοβουλίου. Το ΠΑΣΟΚ κυριαρχεί με 172 έδρες, η ΝΔ έχει 115 και το ΚΚΕ με 13."
...
#| tbl-cap: "Σύνθεση Ελληνικού Κοινοβουλίου Γ´ περιόδου (1981 - 1985)"
...
#| fig-alt: "Ημικυκλικό γράφημα κοινοβουλίου. Η ΝΔ πρώτη με 145 έδρες, το ΠΑΣΟΚ δεύτερο με 125. Πρώτη εμφάνιση του Συνασπισμού ως τρίτης δύναμης με 28 έδρες. Επιπλέον ένας ανεξάρτητος βουλευτής και ένας της ΔΗΑΝΑ (Δημοκρατικής Ανανέωσης)"
...
#| tbl-cap: "Σύνθεση Ελληνικού Κοινοβουλίου ΙΓ' περιόδου (2009 - 2012)"
...
#| fig-cap: "Σύνθεση Ελληνικού Κοινοβουλίου ΙΕ' περιόδου (2012 - 2014)"
...
#| tbl-cap: "Σύνθεση Ελληνικού Κοινοβουλίου ΙΕ' περιόδου (2012 - 2014)"
...
#| fig-cap: "Σύνθεση Ελληνικού Κοινοβουλίου ΙΖ' περιόδου (2015-2019)"
...
#| tbl-cap: "Σύνθεση Ελληνικού Κοινοβουλίου ΙΖ' περιόδου (2015 - 2019)"
...
#| tbl-cap: "Αριθμός κοινοβουλευτικών ομάδων και ανεξάρτητων ανά κοινοβουλευτική θητεία"
clean_dataset %>%
    distinct(period_num, period_name, dates, abbrNamesEl) %>%
    count(period_num, period_name, dates, name = "n") %>%
    arrange(period_num) %>%
...
#| fig-cap: "Βουλευτές που έχουν εκλεγεί περισσότερες φορές εκπροσωπώντας συγκεκριμένο κόμμα"
#| fig-height: 14
...
#| fig-cap: "Βουλευτές που έχουν εκλεγεί περισσότερες φορές με το Κομμουνιστικό Κόμμα Ελλάδος (ΚΚΕ)"
#| fig-height: 8
party_plot(
...
::: {.column-margin}
![Κωνσταντίνος Μητσοτάκης, ~1950](images/mitso.webp){fig-alt="Ο Κωνσταντίνος Μητσοτάκης σε ασπρόμαυρη φωτογραφία"}
...
#| fig-cap: "Βουλευτές που έχουν εκλεγεί περισσότερες φορές με τη Νέα Δημοκρατία (ΝΔ)"
#| fig-height: 10
party_plot(
  dataset = clean_dataset,
...
#| fig-cap: "Βουλευτές που έχουν εκλεγεί περισσότερες φορές με το Πανελλήνιο Σοσιαλιστικό Κίνημα (ΠΑΣΟΚ)"
#| fig-height: 10
party_plot(
  dataset = clean_dataset,
...
Ο Συνασπισμός της Αριστεράς και της Προόδου ιδρύθηκε το 1989 ως εκλογική συμμαχία του ΚΚΕ και της ΕΑΡ (μετεξέλιξη του ΚΚΕ εσωτερικού) αλλά και άλλων μικρότερων κομμάτων του κεντροαριστερού χώρου. Το 1991 έγινε ενιαίο κόμμα απορροφώντας τα κόμματα που αποτελούσαν την συμμαχία. Μέχρι το 2012 παρέμεινε μία μικρή κοινοβουλευτική δύναμη, με ποσοστά που σπανίως ξεπερνούσαν το 5%. Η εκρηκτική άνοδός του συνέπεσε με την οικονομική κρίση: από 4,6% (2009) σε 26,9% (Ιούνιος 2012) και τελικά 36,3% (Ιανουάριος 2015), κερδίζοντας τις εκλογές. Υπό τον Αλέξη Τσίπρα, κυβέρνησε σε συνεργασία με τους Ανεξάρτητους Έλληνες (ΑΝΕΛ) και, παρά τη ρητορική κατά της λιτότητας, υπέγραψε τελικά τρίτο Μνημόνιο. Στις εκλογές του 2019 ηττήθηκε, ενώ στις εκλογές του 2023 υπέστη σημαντικές απώλειες. Η ιδιαιτερότητα του ΣΥΡΙΖΑ από αναλυτικής σκοπιάς είναι ότι εκτινάχθηκε από μικρές ποσοστιαίες μονάδες στον ρόλο αξιωματικής αντιπολίτευσης και κυβέρνησης μέσα σε λίγα χρόνια.

::: {.column-margin}
![Νίκος Κωνσταντόπουλος, 2013](images/Konstantopoulos.webp){fig-alt="Ο Νίκος Κωνσταντόπουλος, έγχρωμη φωτογραφία"}

Ο Νίκος Κωνσταντόπουλος (Ηλεία, 1942 - ) ήταν Έλληνας πολιτικός και δικηγόρος. Διετέλεσε πρόεδρος του Συνασπισμού της Αριστεράς και της Προόδου από το 1993 μέχρι και το 2004.
<br>
**Πηγή φωτογραφίας: **Wikimedia - Φωτογραφία του 2013 - Άδεια χρήσης: [CC BY 3.0](https://creativecommons.org/licenses/by/3.0/). Η πρωτότυπη εικόνα βρέθηκε στο [Europeana](https://commons.wikimedia.org/wiki/File:Nikos_Konstantopoulos_2013.jpg)
:::

```{r}
#| label: fig-most-elected-mps-syriza
#| fig-cap: "Βουλευτές που έχουν εκλεγεί περισσότερες φορές με το Συνασπισμό Ριζοσπαστικής Αριστεράς (ΣΥΡΙΖΑ)"
#| fig-height: 8
party_plot(
  dataset = clean_dataset,
...
# Κάθε σημείο φέρει: περίοδο (x), ποσοστό (y), διάρκεια (tooltip)
series_data_f <- lapply(1:nrow(f), function(i) {
    list(
        name  = f$period_name[i],
        y     = f$pctSeatsTopTwoParties[i],
        dates = f$dates[i]
    )
})
...
highchart() %>%
    hc_chart(type = "spline") %>%
    hc_title(
        text  = "Εκπροσώπηση δικομματισμού",
        style = list(fontWeight = "bold")
    ) %>%
    hc_subtitle(
        text = "Ποσοστό (%) εδρών που κατέχουν τα δύο πρώτα κόμματα ανά θητεία"
    ) %>%
    hc_xAxis(
        type  = "category",
        title = list(text = "Βουλευτική Περίοδος"),
        labels = list(
            rotation = -45,
    ...
    hc_yAxis(
        title       = list(text = "Ποσοστό εδρών δύο πρώτων κομμάτων (%)"),
        gridLineColor = "#E6E6E6",
        gridLineWidth = 1,
        max = 100,
        min = 0
    ...
    hc_tooltip(
        ...
        headerFormat = "",
        pointFormat  = "
            <b>Περίοδος:</b> {point.name}<br/>
            <b>Διάρκεια:</b> {point.dates}<br/>
            <b>Ποσοστό εδρών:</b> <b>{point.y}%</b>
        ",
...
test1 <- test |>
    group_by(period_num, period_name, dates, gender) |>  # <-- προσθήκη
    count() |>
    pivot_wider(names_from = gender, values_from = n, values_fill = 0)

# Helper για series με tooltip info
    ...
    hc_tooltip(
        ...
        formatter = JS("
            function() {
                ...
                var s = '<b>Περίοδος: ' + periodName + '</b><br/>';
                s    += '<span style=\"color:#666\">Διάρκεια: ' + dates + '</span><br/><br/>';
...
```{r}
# Προσθήκη period_name και dates στο dataset
test = clean_dataset |> select(period_num, period_name, dates, gender, abbrNamesEl)

test1 = test |>
  group_by(period_num, period_name, dates, gender, abbrNamesEl) |>
  count() |>
  ...
  ungroup() %>%
  select(period_num, period_name, dates, abbrNamesEl, pct) %>%
...
# Βοηθητική συνάρτηση — ίδια λογική με τα υπόλοιπα γραφήματα του άρθρου
make_party_series <- function(values, df) {
  lapply(1:nrow(df), function(i) {
    list(
      name        = df$period_name[i],   # γράμματα θητείας στον x-άξονα
      y           = values[i],
      dates       = df$dates[i],
...
highchart() %>%
  ...
  hc_subtitle(
    text = "Η Νέα Δημοκρατία ποτέ στην ιστορία της δεν εκπροσωπήθηκε από γυναίκες
            σε υψηλότερο του 17%, σε αντίθεση με άλλα ιστορικά κόμματα."
  ) %>%
  hc_xAxis(
    type       = "category",
    categories = test1$period_name,          # γράμματα αντί αριθμών
    title      = list(text = "Κοινοβουλευτική Περίοδος")
  ) %>%
  hc_yAxis(
  ...
  hc_add_series(
    name      = "ΚΚΕ",
    data      = make_party_series(test1$ΚΚΕ, test1),
    color     = "#FF6666",
    lineWidth = 3,
  ...
  hc_add_series(
    name      = "ΝΔ",
    data      = make_party_series(test1$ΝΔ, test1),
    color     = "#0492c2",
    lineWidth = 3,
    marker    = list(radius = 5, symbol = "circle")
  ) %>%
  hc_add_series(
    name      = "ΠΑΣΟΚ",
    data      = make_party_series(test1$ΠΑΣΟΚ, test1),
    color     = "#95bb72",
    lineWidth = 3,
    marker    = list(radius = 5, symbol = "circle")
  ) %>%
  hc_add_series(
    name      = "ΣΥΡΙΖΑ",
    data      = make_party_series(test1$ΣΥΡΙΖΑ, test1),
    color     = "#e27bb1",
    lineWidth = 3,
    marker    = list(radius = 5, symbol = "circle")
  ) %>%
...
Σε αυτή τη περίπτωση μελετάμε τη περίοδο 1981 - 2019 (Γ' περίοδος - ΙΖ´ περίοδος), επομένως η σύγκριση έχει νόημα να ξεκινήσει από τη περίοδο Δ' (1985), ώστε να να έχω προηγούμενη περίοδο σύγκρισης. Στο διάγραμμα διακρίνονται ισχυρές τάσεις διατήρησης όμοιων προσώπων με μόνο δύο εκλογικές αναμετρήσεις που οδήγησαν σε σημαντική ανανέωση του κοινοβουλίου αντικαθιστώντας ουσιαστικά τα μισά μέλη με νεοεκλεγέντες.
...
# Τρέχουμε τη συνάρτηση όπως πριν
results <- lapply(2:17, get_new_members)

# Ενώνουμε τα αποτελέσματα με την πληροφορία περιόδου
df <- data.frame(
    num         = 2:17,
...
# Series data — κάθε σημείο φέρει period_name και dates
...
highchart() %>%
    hc_chart(type = "areaspline") %>%
    hc_title(
        text  = "Κοινοβουλευτική ανανέωση ανά θητεία",
        style = list(fontWeight = "bold")
    ) %>%
    hc_subtitle(
        text = "Ποσοστό βουλευτών που εκλέγονται για πρώτη φορά σε κάθε θητεία"
    ) %>%
    hc_xAxis(
        type  = "category",
        title = list(text = "Βουλευτική Περίοδος"),
    ...
    hc_tooltip(
        ...
        formatter = JS("
            function() {
                ...
                // Παίρνουμε period_name και dates από το πρώτο point
                var periodName = this.points[0].point.name;
                var dates      = this.points[0].point.dates;
                ...
                for (var i = 0; i < this.points.length; i++) {
                    ...
                    s += '<span style=\"color:' + this.points[i].color + '\">●</span> ' +
                         '<b>' + this.points[i].series.name + ':</b> ' +
                         this.points[i].y + ' βουλευτές (' + pct + '%)<br/>';
                }
                return s;
            }
        "),
