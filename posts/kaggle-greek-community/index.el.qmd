...
```{r}
# install.packages(c("highcharter", "dplyr"))
library(highcharter)
library(dplyr)

# --- Example data: 35 countries with values ---
set.seed(123)
df <- tibble::tibble(
  country = paste("Country", 1:35),
  value   = round(runif(35, 500, 5000))
)
...
highchart() %>%
  ...
  hc_title(text = "Country Performance") %>%
  hc_subtitle(text = "Descending order, color intensity reflects value") %>%
  hc_xAxis(
    categories = df_sorted$country,
  ...
  hc_add_series(
    ...
    colorByPoint = TRUE,
    dataLabels = list(enabled = FALSE) # ⬅ disables labels after bars
  ) %>%
  hc_colorAxis(
    minColor = "#e0f3f8",
    maxColor = "#08306b",
  ...
  hc_tooltip(
    useHTML = TRUE,
    pointFormat = "<span style='font-weight:600'>{point.category}</span><br/>Value: <b>{point.y:,.0f}</b>"
  ) %>%
  hc_legend(enabled = FALSE) %>%
...
Η κοινότητα των Ελλήνων χρηστών αποτελείται από μεγαλύτερης ηλικίας άτομα σε σχέση με την υπόλοιπη κοινότητα. Η πολυπληθέστερη ηλικικακή ομάδα στην Ελλάδα είναι οι 25-29 ετών, ενώ στην υπόλοιπη πλατφόρμα 
