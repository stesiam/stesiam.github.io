...
```{r}
adf_results = adf.test(grc_unemployment$Value)

data = data.frame(c("Dickey-Fuller", adf_results$statistic), c("lag order", 6), c("p-value", adf_results$p.value)) %>% as.data.frame()
...
```{r}
adf.test(grc_unemployment_diff1)
```
