...
unemployment_ts <- unemployment_ts %>%
  mutate(
    break1 = if_else(TIME >= break_date_1 & TIME < break_date_2, 1, 0),
    break2 = if_else(TIME >= break_date_2, 1, 0)
  )
...
```{r}
#| label: tbl-rmse-models
#| tbl-cap: "Σύγκριση σφαλμάτων διαφορετικών μοντέλων"
fit <- cv %>%
  model(
    "Auto ARIMA"   = fable::ARIMA(Value),  # ← Force d=1!
    "ARIMA D1" = fable::ARIMA(Value ~ pdq(d = 1)),
    ARIMAX = fable::ARIMA(Value ~ pdq(d = 1) + break1 + break2),
    ARIMA516 = fable::ARIMA(Value ~ pdq(p = 5, d = 1, q = 6), order_constraint = 12),
    ETS     = fable::ETS(Value),
