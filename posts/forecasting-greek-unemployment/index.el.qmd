...
auto_model <- auto.arima(grc_unemployment$Value, seasonal = TRUE, d = 1, trace = TRUE, stepwise = FALSE, max.p = 11, max.q = 11,max.order = 30, ic = "bic")
...
arimaModel_2=arima(grc_unemployment$Value, order=c(1,1,1))
arimaModel_3=arima(grc_unemployment$Value, order=c(2,1,1))
arimaModel_4=arima(grc_unemployment$Value, order=c(1,1,2))
arimaModel_4=arima(grc_unemployment$Value, order=c(2,1,2))
arimaModel_3=arima(grc_unemployment$Value, order=c(3,1,1))
arimaModel_4=arima(grc_unemployment$Value, order=c(3,1,2))
arimaModel_4=arima(grc_unemployment$Value, order=c(3,1,3))
...
accuracy_table %>% gt_custom(head_max = 8)
```
...
```{r}
y <- ts(data= grc_unemployment$Value, start = c(1998,4), frequency=12)
...
results = lapply(combsETS, function(m) {
    ets(y, model = m)
})
aic_values <- sapply(results, function(fit) {
     if (is.null(fit)) NA else AIC(fit)
 })

bic_values <- sapply(results, function(fit) {
     if (is.null(fit)) NA else BIC(fit)
 })

data.frame(
  "Μοντέλο" = combsETS,
  "Κριτήριο AIC"  = aic_values,
  "Κριτήριο BIC" = bic_values,
  check.names = FALSE
) %>%
  gt_custom()
```
