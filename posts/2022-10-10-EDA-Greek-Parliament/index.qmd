...
```{r}
plot_parliament_term <- function(term_num, parliament_data, parties_data) {
  ...
  term_data_joined <- fuzzyjoin::stringdist_left_join(term_data, parties_data, by = "Party",
                                                      max_dist = 2, distance_col = "distance") |>
    group_by(Party.x) |>
    dplyr::filter(distance == min(distance)) |>
    rename(Party = Party.x) |>
    select(Party, n, Color)
  ...
  highcharter::hchart(
    ...
    hc_title(text = "Greek Parliament") %>%
    hc_subtitle(text = paste("Η σύνθεση της Βουλής για την Θητεία", term_num)) |>
    hc_legend(labelFormat = '{name} <span style="opacity: 0.4">{y}</span>')
}
```

```{r}
plot_parliament_term(3,parliament, parties)
```
...
```{r}
plot_parliament_term(4,parliament, parties)
```
...
```{r}
plot_parliament_term(9,parliament, parties)
```
