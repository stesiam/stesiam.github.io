...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
	...
	\hline
	\makecell{\textbf{Negative Binomial 0} \\ $NB_0(r,p)$ \# successes} & \makecell{$r = 0, 1, 2, \ldots$  \\  $p \in [0,1]$} &  &  &  & $\frac{rq}{p}$ & $\frac{rq}{p^2}$ & $\left(\frac{p}{1-qe^t}\right)^r$ & $\left(\frac{p}{1-qt}\right)^r$ \\
	\hline
	\makecell{\textbf{Negative Binomial 1} \\ $NB_1(r,p)$} & \makecell{$r = 1,2, \ldots$  \\  $p \in [0,1]$} &  &  &  &  &  &  &  \\
	\hline
...
\begin{tabular}{|c|c|c|c|c|c|c|c|}
	...
	\hline
	\makecell{\textbf{Weibull} \\ $Wei(\lambda,k)$} & \makecell{$\lambda \in (0,+\infty)$ \\ $k \in (0,+\infty)$} & $\begin{cases} \frac{k}{\lambda}(\frac{x}{\lambda}) e^{-(\frac{x}{\lambda})^k}) , & x \geq 0 \\ 0 , & x<0 \end{cases}$ & $x \in [0,\infty]$ & $\begin{cases} 1-e^{-(\frac{x}{\lambda})} , & x \leq 0 \\ 0 , & x<0 \end{cases}$ & $\lambda$ Γ$(1 + \frac{1}{k})$ &  &  \\
	\hline
